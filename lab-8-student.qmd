...
fish |>
  ...
  summarise(frq_na = sum(is.na(weight))) |>
  ggplot(mapping = aes(x = year, 
                       y = frq_na,
                       colour = section)) +
  geom_point() +
  labs(title = "Frequency Of Missing Values in Fish Data", 
       x= "Year",
...
> Divided the max of weight by the min of length rather than by weight!
...
rescale_01 <- function(x, min, max) {
  if(!is.numeric(x)) {
    stop("Input must be numeric") #https://www.geeksforgeeks.org/how-to-validate-input-to-a-function-error-in-r/, adding conditions :)
  }
  if(length(x) < 1) {
    stop("Length must be greater than 1")
  }
